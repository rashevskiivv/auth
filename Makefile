...
deploy-pg:
	cd deployment && docker-compose ${DC_FILE} -p ${RELEASE_NAME} up -d auth-db postgres_migrate
...
delete-pg:
	cd deployment && docker-compose ${DC_FILE} -p ${RELEASE_NAME} rm -sf auth-db  postgres_migrate
